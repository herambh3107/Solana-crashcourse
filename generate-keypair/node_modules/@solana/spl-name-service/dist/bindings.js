 ...
 * @returns
 */
async function reallocNameAccount(connection, name, space, payerKey, nameClass, nameParent) {
    const hashedName = await (0, utils_2.getHashedName)(name);
    const nameAccountKey = await (0, utils_2.getNameAccountKey)(hashedName, nameClass, nameParent);
    let nameOwner;
    if (nameClass) {
        nameOwner = nameClass;
    }
